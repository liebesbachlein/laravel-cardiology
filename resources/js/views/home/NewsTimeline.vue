<template>
    <div class="newstimeline">
        ...
        <MonthBlock id="m1" month="Май 2024" :lightUp="ms[0]" style="position: relative"/>
        <div class="month-news">
            <NewsMiniRight :event="news[0]"/>
        </div>
        <MonthBlock id="m2" month="Июнь 2024" :lightUp="ms[1]" style="position: relative; "/>
        <!--<div class="month-news">
...
<!--<template>
    <div class="newstimeline">
        <div class="site-content-in" id="newsTimeline">
            ...
            <div v-for="event1, i in events" :key="i">
                <MonthBlock :id="'m' + i" :month="months[event1[0].month - 1] + ' 2024'" :lightUp="ms[i]" style="position: relative"/>
                <div v-for="event2 in event1" class="month-news">
                    <NewsMiniRight :event="event2"/>
                </div>
                <MonthBlock v-if="i == events.length - 1" :id="'m' + events.length" :month="months[events[events.length - 1][0].month] + ' 2024'" :lightUp="ms[events.length]" style="position: relative; "/>
            </div>
...
onMounted(() => {
    window.addEventListener('scroll', () => {
        if (document.getElementById('m0')) {
            offsets0 = [document.getElementById('m0').getBoundingClientRect().top, document.getElementById('m0').getBoundingClientRect().bottom]
            offsets1 = [document.getElementById('m1').getBoundingClientRect().top, document.getElementById('m1').getBoundingClientRect().bottom]
            circleOffset = document.getElementById('circle').getBoundingClientRect().top
        }
    }) 
})
const ms = computed (() => {
    let ms = [false, false]
    const dis12 = Math.abs(offsets0[0] - offsets1[0])

    if (Math.abs(offsets0[0] - circleOffset) < 50 || Math.abs(offsets0[1] - circleOffset) <= dis12 / 2) {
        ms[0] = true
        ms[1] = false
        if(Math.abs(offsets0[0] - circleOffset) <= 10 || Math.abs(offsets0[2] - circleOffset) <= 10) {
            hideCircle.value = true
        } else {
            hideCircle.value = false
        }
    } else if (Math.abs(offsets1[0] - circleOffset) < dis12 / 2 || Math.abs(offsets1[1] - circleOffset) < 50) {
        ms[0] = false
        ms[1] = true
        hideCircle.value = false
    } 
