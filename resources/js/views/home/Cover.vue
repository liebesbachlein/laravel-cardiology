<template>
    <div v-if="numberOfSlides > 1" class="carousel-arrow carousel-arrow-left">
        <div
            class="carousel-arrow-inner"
            :class="lightMode ? 'carousel-arrow-light-mode' : ''"
            @click="reorderLeft()"
            @mouseover="clearOldInterval"
    ...
    <div class="carousel">
        <div v-if="numberOfSlides > 1" class="carousel-inner">
            <component
                class="carousel-transition poster-transition"
                :is="components[numberOfSlides - 1]"
                :id="`slide${numberOfSlides - 1}add`"
                @lightMode="setLightMode(numberOfSlides - 1)"
                @darkMode="setDarkMode(numberOfSlides - 1)"
            />
            ...
            <component
                ...
                :id="`slide${i - 1}`"
                :key="i - 1"
                @lightMode="setLightMode(i - 1)"
                @darkMode="setDarkMode(i - 1)"
            />
    ...
    if (startX && movePosX) {
        ...
        if (startX - movePosX > 0) {
            ...
            if (curPos == numberOfSlides - 1) {
                ...
                document.getElementById(
                    `slide${nextIndexRight(curPos)}add`
                ).style.transform = `translateX(${
                    (1 - (startX - movePosX) / width) * 100
                }%)`;
            } else {
                document.getElementById(
                    `slide${curPos}`
            ...
            if (curPos == 0) {
                ...
                document.getElementById(
                    `slide${nextIndexLeft(curPos)}add`
                ).style.transform = `translateX(${
                    ((movePosX - startX) / width - 1) * 100
                }%)`;
            } else {
                document.getElementById(
                    `slide${curPos}`
...
function reorderLeft() {
    if (curPos == 0) {
        document.getElementById(
            `slide${numberOfSlides - 1}add`
        ).style.transform = `translateX(${0}%)`;
        document.getElementById(`slide${numberOfSlides - 1}add`).style.zIndex =
            "2";

        document.getElementById(`slide${numberOfSlides - 1}`).style.zIndex =
            "1";
        document.getElementById(
...
async function reorder(nextPos, isTimeout = false) {
    ...
    const loopPoster = function (nextPos) {
        ...
        for (; posterIndex < numberOfSlides; posterIndex++, posterTrans++) {
            document.getElementById(
                `slide${posterIndex}`
            ).style.transform = `translateX(${posterTrans * 100}%)`;

            if (nextPos == posterIndex) {
                document.getElementById(`slide${posterIndex}`).style.zIndex =
                    "2";
            } else {
                document.getElementById(`slide${posterIndex}`).style.zIndex =
                    "1";
            ...
            if (posterIndex == 0 || posterIndex == numberOfSlides - 1) {
                if (nextPos == posterIndex) {
                    ...
                    document.getElementById(
                        ...
                        (posterIndex
                            ? posterTrans - numberOfSlides
                            : numberOfSlides + posterTrans) * 100
                    }%)`;
                }
                document.getElementById(`slide${posterIndex}add`).style.zIndex =
                    "0";
            }
        }
    };
